import type { IBasicAnimation, ICompositionAnimation, ITrackItem } from "@designcombo/types";
import { Easing } from "remotion";
import type { Animation } from "../player/animated/types";
		...
		animation.in.composition.forEach((comp) => {
			if (animation.in.name.includes("slide")) {
				...
				animationIn.push({
					...
					ease: Easing[comp.easing as keyof typeof Easing] as (t: number) => number,
				});
			}
		});
	}
...
const getSlideAnimation = (type: string, anim: ICompositionAnimation, item: ITrackItem) => {
	...
	if (type === "slideInRight" || type === "slideOutLeft") {
		const commonValue = -Number.parseFloat(item.details.left) - item.details.width / scale;
	...
	if (type === "slideInBottom" || type === "slideOutTop") {
		const commonValue = -Number.parseFloat(item.details.top) - item.details.height / scale;
		const from = type.includes("In") ? commonValue : anim.from;
		const to = type.includes("In") ? anim.to : commonValue;
		return {
			property: anim.property,
