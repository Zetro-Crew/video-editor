import { type Control, Trimmable, type TrimmableProps, timeMsToUnits } from "@designcombo/timeline";
...
import { createAudioControls } from "../controls/controls";
...
class Audio extends Trimmable {
	...
	public hasSrc = true;
	public metadata?: Partial<IMetadata> & { previewUrl: string; displayName?: string };
	...
	public drawTextIdentity(ctx: CanvasRenderingContext2D) {
		...
		ctx.textAlign = "left";
		ctx.clip();
		ctx.fillText(this.metadata?.displayName ?? "Audio", 36, 14);
		ctx.translate(8, 1);
		ctx.fillStyle = "#ffffff";
