...
import { getAudioData, type MediaUtilsAudioData, visualizeAudio } from "@remotion/media-utils";
...
class AudioDataManager {
	...
	public async setItems(items: (ITrackItem & (IVideo | IAudio))[]) {
		...
		const removeItemIds = currentItemIds.filter((id) => !newItemIds.includes(id));
	...
	public validateUpdateItems(validateItems: (ITrackItem & (IVideo | IAudio))[]) {
	...
	private combineValues = (length: number, sources: Array<number[]>): number[] => {
	...
	getAudioDataForFrame(frame: number): number[] {
		...
		const result = this.combineValues(this.numberOfSamples, visualizationValues);
