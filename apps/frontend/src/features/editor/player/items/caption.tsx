...
export default function Caption({
	...
	const { transformStyles, globalOpacity, extraStyles } = calculateAnimationTransforms(
		updatedDetails,
		frame || 0,
		from,
		fps,
	...
	const { height, width, ...filteredStyles } = calculateContainerStyles(updatedDetails);
...
function calculateAnimationTransforms(
	...
	if (updatedDetails?.animation) {
		...
		if (config.translate || userConfig.translate) {
			...
			const translation = TranslateAnimationCaption(relativeFrame, translateConfig);
		...
		if (config.scalePulse || userConfig.scalePulse) {
			transformValues.scale = ScalePulseAnimationCaption(relativeFrame, item.display.to, fps);
		}
		...
		if (config.translateOnceAnimation || userConfig.translateOnceAnimation) {
			...
			if (translateOnceConfig) {
				const { duration = 30, orientation = "horizontal" } = translateOnceConfig;
				transformValues = {
					...transformValues,
		...
		if (config.rotateRandom || userConfig.rotateRandom) {
			...
			if (!captionRotationCache.has(captionId)) {
				...
				if (captionRotationCache.size >= 500) {
					captionRotationCache.delete(captionRotationCache.keys().next().value!);
				}
		...
		if (config.rotateFixed !== undefined || userConfig.rotateFixed !== undefined) {
			transformValues.rotate = userConfig.rotateFixed ?? config.rotateFixed;
		}

		// Apply global opacity
		if (config.globalOpacity !== undefined || userConfig.globalOpacity !== undefined) {
			const globalOpacityConfig = userConfig.globalOpacity ?? config.globalOpacity;
...
function parseUserAnimationConfig(animationString: string): AnimationConfig {
	...
	animationsUser.forEach((anim) => {
		...
		if (anim.startsWith("translate-")) {
			...
			if (option === "horizontal" || option === "vertical" || option === "bilateral") {
...
function renderWords(
	...
	return renderStandardWords(item, updatedDetails, scaleFactor, offsetFrom, globalOpacity);
}
...
function renderLineBasedWords(
	...
	const wordsPerLine = Math.ceil(item.details.words.length / updatedDetails.linesPerCaption);
	const lines: any[][] = [];

	for (let i = 0; i < updatedDetails.linesPerCaption; i++) {
		const startIndex = i * wordsPerLine;
		const endIndex = Math.min(startIndex + wordsPerLine, item.details.words.length);
		lines.push(item.details.words.slice(startIndex, endIndex));
	}

	const lineDuration = (item.display.to - item.display.from) / updatedDetails.linesPerCaption;
...
function renderCustomAnimation1Words(
	...
	const nonKeywordWords = item.details.words.filter((word: any) => !word.is_keyword);
	const keywordWords = item.details.words.filter((word: any) => word.is_keyword);
	...
	if (nonKeywordWords.length > 0) {
		...
		const groupEndTime = firstKeywordWord ? firstKeywordWord.start : lastNonKeyword.end;
