...
const createInterpolationHelpers = (
	currentFrame: number,
	startAtFrame: number,
	endAtFrame: number,
) => {
	...
	const handleMidFrameAnimation = (
		...
		if (currentFrame <= midFrame) {
			result.scale = interpolate(currentFrame, [startAtFrame, midFrame], [scaleFrom, scaleTo], {
				extrapolateLeft: "clamp",
				extrapolateRight: "clamp",
			});
			...
			result.scale = interpolate(currentFrame, [midFrame, endAtFrame], [scaleTo, scaleFrom], {
				extrapolateLeft: "clamp",
				extrapolateRight: "clamp",
			});
		}
...
export const createAnimationFunctions = (
	...
	const { interpolateScale, interpolateOpacity, interpolatePulse, handleMidFrameAnimation } =
	...
	const scaleAnim = (from: number, to = 1) => ({
		scale: interpolateScale(from, to),
	});

	const translateXAnim = (from: number, to = 0) => ({
		translateX: interpolateScale(from, to),
	});

	const translateYAnim = (from: number, to: number, easing?: (t: number) => number) => ({
		translateY: interpolate(currentFrame, [startAtFrame, endAtFrame], [from, to], {
			extrapolateLeft: "clamp",
			extrapolateRight: "clamp",
			easing: easing || ((t) => t),
		}),
...
export const ANIMATION_CONFIGS: Record<
	string,
	(helpers: ReturnType<typeof createAnimationFunctions>) => Partial<WordAnimationState>
> = {
