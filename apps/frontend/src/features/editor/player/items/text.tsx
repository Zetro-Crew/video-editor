...
export default function Text({ item, options }: { item: IText; options: SequenceItemOptions }) {
	...
	const animationTextInFrames = animations?.in?.composition[0].durationInFrames || 0;
	const animationTextOutFrames = animations?.out?.composition[0].durationInFrames || 0;
	const animationTextLoopFrames = animations?.loop?.composition[0].durationInFrames || 0;
