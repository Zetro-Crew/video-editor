...
import type { ICaption, ITrackItem, ITrackItemsMap } from "@designcombo/types";
...
function regroupCaptions(captions: ICaption[], newLinesPerCaption: number): ICaption[] {
...
function transformCaptions(captions: ICaption[], type: CaptionTransformType): ICaption[] {
	...
	switch (type) {
		...
		case "punctuationOrPause": {
			...
			for (let i = 0; i < allWords.length; i++) {
				...
				if (!shouldSplit && (/[.,!?;]/.test(word.word) || word.word.endsWith("."))) {
					shouldSplit = true;
				}
		...
		case "time": {
			...
			while (
				...
				while (wordIndex < allWords.length && allWords[wordIndex].start < currentEnd) {
...
const CaptionWords = ({ trackItem }: { id: string; trackItem: ITrackItem & any }) => {
	const { trackItemsMap, size } = useCompositionStore();
	...
	return (
		<div className="flex flex-col gap-2 py-4">
			...
			<div className="flex gap-2">
				<div className="flex flex-1 items-center text-sm text-muted-foreground">Lines per Page</div>
						...
						<PopoverContent className="z-[300] w-32 p-0">
							{OPTIONS_LINES_PER_PAGE.map((option, index) => (
								<Button
									...
									className="w-full"
									key={index}
									onClick={() => onChange({ type: "linesPerCaption", value: option.value })}
								>
									{option.label}
								</Button>
							))}
			...
			<div className="flex gap-2">
				<div className="flex flex-1 items-center text-sm text-muted-foreground">Words per line</div>
				<div className="flex gap-2">
					<div className="relative w-32">
						<Popover>
							<PopoverTrigger asChild>
							...
							<PopoverContent className="z-[300] w-32 p-0">
								{OPTIONS_WORDS_PER_LINE.map((option, index) => (
									<Button
										...
										key={index}
										onClick={() => onChange({ type: "wordsPerLine", value: option.value })}
									>
										{option.label}
									</Button>
								))}
			...
			<div className="flex gap-2">
				<div className="flex flex-1 items-center text-sm text-muted-foreground">Words in line</div>
				<div className="flex gap-2">
					<div className="relative w-32">
						<Popover>
							<PopoverTrigger asChild>
							...
							<PopoverContent className="z-[300] w-32 p-0">
								{OPTIONS_WORDS_IN_LINE.map((option, index) => (
									<Button
										...
										className="w-full truncate"
										key={index}
										onClick={() => onChange({ type: "showObject", value: option.value })}
									>
										{option.label}
									</Button>
								))}
			...
			<div className="flex gap-2">
				<div className="flex flex-1 items-center text-sm text-muted-foreground">Position</div>
				<div className="flex gap-2">
					<div className="relative w-32">
						<Popover>
							<PopoverTrigger asChild>
			...
			<div className="flex gap-2">
				<div className="flex flex-1 items-center text-sm text-muted-foreground">Transition</div>
				<div className="relative w-32">
					<Popover>
						<PopoverTrigger asChild>
							<Button
								...
								<div className="w-full overflow-hidden text-left">
									<p className="truncate">
										{selectedOptions.length === 0
											? "None"
											: animationOptions.find((opt) => opt.key === selectedOptions[0])?.label ||
												"None"}
									</p>
								</div>
								<ChevronDown className="text-muted-foreground" size={14} />
						...
						<PopoverContent className="w-48 p-2">
							<div className="space-y-1">
								{animationOptions.map((opt) => (
									<Button
										key={opt.key}
										variant={selectedOptions.includes(opt.key) ? "default" : "ghost"}
										size="sm"
