...
const ControlAllItems: React.FC<ControlAllItemsProps> = ({ type, handleAllClick }) => {
	return (
		<>
			{type?.type &&
				{
					visualizers: <AllVisualizers handleAllClick={handleAllClick} type={type} />,
				}[type.type]}
		</>
	);
};
...
const AllVisualizers = ({
	...
	const [selectedCategoryVisualizers, setSelectedCategoryVisualizers] = useState<string | null>(
		null,
	);
	...
	const selectedGroup =
		type.all !== "all" ? visualizerGroups.find((group) => group.category === type.all) : null;
	return (
		<div className="flex flex-col gap-4 flex-1 max-w-full">
			<div
				className={`flex justify-between items-center px-4 ${type.all === "all" ? "" : "border-b"}`}
			>
						...
						<HorizontalScroll className="w-full" debug={false}>
							<div className="flex gap-2 pb-2 min-w-max">
								<button
									...
									className={cn(
										buttonVariants({
											variant: selectedCategoryVisualizers === null ? "default" : "outline",
										}),
										"h-7 cursor-pointer",
									)}
								>
								...
								{VISUALIZERS_TAGS.map((tag) => (
									<button
										...
										className={cn(
											buttonVariants({
												variant: selectedCategoryVisualizers === tag.id ? "default" : "outline",
											}),
											"h-7 cursor-pointer",
										)}
									>
			...
			<div className="px-4">
				{type.all === "all" ? (
					selectedCategoryVisualizers === null ? (
							...
							{visualizerGroups.map((group) => (
								<div key={group.category} className="flex flex-col gap-4">
									<div className="flex justify-between items-center">
										...
										<div
											className="text-xs text-muted-foreground flex cursor-pointer"
											onClick={() => handleCategoryVisualizersClick(group.category)}
										>
											View all
											<ChevronRight className="ml-1 h-4 w-4" />
										</div>
						...
						renderItems(
							visualizerGroups.find((group) => group.category === selectedCategoryVisualizers)!,
						)
					)
				) : (
