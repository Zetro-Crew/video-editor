...
export const ElementCrop: React.FC<ElementCropProps> = ({
				...
				if (dirY === -1) {
					...
					if (ratio) {
						...
						if (newHeight >= MIN_CROP_SIZE) {
							...
							if (newWidth >= MIN_CROP_SIZE && newArea[0] + newWidth <= size.width * scale) {
								newArea[1] = relativeY;
								newArea[3] = newHeight;
								newArea[2] = newWidth;
							}
				...
				if (dirX === -1) {
					...
					if (ratio) {
						...
						if (newWidth >= MIN_CROP_SIZE) {
							...
							if (newHeight >= MIN_CROP_SIZE && newArea[1] + newHeight <= size.height * scale) {
								newArea[0] = relativeX;
								newArea[2] = newWidth;
								newArea[3] = newHeight;
							}
	...
	useEffect(() => {
		...
		const { scale: initialScale, size: initialSize } = useCropStore.getState();
		const areaPosX = (targetDetails.crop?.x || 0) * initialScale;
		const areaPosY = (targetDetails.crop?.y || 0) * initialScale;
		const areaWidth = (targetDetails.crop?.width || initialSize.width) * initialScale;
		const areaHeight = (targetDetails.crop?.height || initialSize.height) * initialScale;
	...
	useEffect(() => {
		...
		const clampedY = Math.max(0, Math.min(newY, size.height * scale - newHeight));
