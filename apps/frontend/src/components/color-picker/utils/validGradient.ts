...
const combineRegExp = (regexpList: ReadonlyArray<string | RegExp>, flags: string): RegExp => {
	return new RegExp(
...
const parseGradient = (regExpLib: IGradientReg, input: string) => {
	...
	if (matchGradient !== null) {
		...
		while (matchColorStop !== null) {
			...
			if (matchColorStop[2]) {
				stopResult.position = Number((Number.parseInt(matchColorStop[2], 10) / 100).toFixed(2));
			}
...
export default (input: string) => {
	...
	let result: IParsedGraient | "Not correct position" | "Failed to find gradient";
	...
	if (match !== null) {
		...
		if (result.stops.every((item) => Object.hasOwn(item, "position")) === false) {
