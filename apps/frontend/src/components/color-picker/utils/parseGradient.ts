...
export default (str: string) => {
	...
	const normalizedGradient = str.replace(";", "").replace("background-image:", "");
	const gradient = validGradient(normalizedGradient);

	let stops: Array<IGradientStop> = [];
	let angle = "";

	if (gradient === "Failed to find gradient" || gradient === "Not correct position") {
	...
	const [, type, content] = normalizedGradient.match(/^(\w+)-gradient\((.*)\)$/i) || [];
	if (!type || !content) {
		console.warn("Incorrect gradient value");
		return defaultStops;
	}
	...
	return {
		...
		type,
		modifier: modifier.match(/\d+/) !== null ? Number(modifier.match(/\d+/)?.join("")) : modifier,
