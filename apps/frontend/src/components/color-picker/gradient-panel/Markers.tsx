...
const Markers: FC<IPropsPanel> = ({
	...
	const onAddColorStop = (e: MouseEvent) => {
		...
		if (target.className !== "gradient-marker") {
			...
			const newStops = [...color.stops, [rgba.toRgbString(), loc, color.stops.length]]
	...
	const deleteColorStop = () => {
		...
		const lastStop = fallbackStop ? (rgbaToArray(fallbackStop[0]) ?? [0, 0, 0, 1]) : [0, 0, 0, 1];
	...
	return (
		<div onClick={(e) => onAddColorStop(e)} ref={node}>
			<GradientStopPreview
				stops={stops.map((color: [string, number]) => `${color[0]} ${color[1] * 100}%`).join(", ")}
			/>
