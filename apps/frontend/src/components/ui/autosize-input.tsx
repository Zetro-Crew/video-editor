...
const AutosizeInput: React.FC<AutosizeInputProps> = (props) => {
	...
	const [inputWidth, setInputWidth] = useState<number>(
		typeof minWidth === "number" ? minWidth : Number.parseInt(minWidth, 10),
	);
		...
		const updateInputWidth = () => {
			...
			const calculatedExtraWidth =
				props.type === "number" && extraWidth === undefined
					? 16
					: Number.parseInt(extraWidth as string, 10) || 0;
			newInputWidth += calculatedExtraWidth;

			if (
				newInputWidth < (typeof minWidth === "number" ? minWidth : Number.parseInt(minWidth, 10))
			) {
				newInputWidth = typeof minWidth === "number" ? minWidth : Number.parseInt(minWidth, 10);
			}
