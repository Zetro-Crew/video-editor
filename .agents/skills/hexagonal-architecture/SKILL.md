...
description: Design, implement, and refactor Ports & Adapters systems with clear domain boundaries, dependency inversion, and testable use-case orchestration in TypeScript/Fastify.
origin: ECC
---
...
Define a single use case with a clear input and output DTO. Keep transport details (Fastify `request`, job payload wrappers) outside this boundary.
...
Instantiate adapters, then inject them into use cases. Keep this wiring centralized (`src/bootstrap/container.ts`) to avoid hidden service-locator behavior.
...
src/
  bootstrap/
    container.ts          ← composition root (buildContainer)
    server.ts             ← Fastify plugin wiring
  features/<name>/
    domain/               ← business rules, no framework imports
    application/
      ports/
        inbound/          ← use-case interfaces
        outbound/         ← repo/gateway/infra interfaces
      use-cases/          ← orchestration classes
    adapters/
      inbound/
        http/             ← Fastify controllers + Zod schemas
      outbound/
        redis/            ← Redis adapters
  infrastructure/
    ffmpeg/               ← FfmpegVideoProcessingAdapter + builders
    storage/              ← S3StorageAdapter
  shared/
    domain/               ← cross-feature types (RenderTypes, VideoMetadata)
    application/ports/outbound/  ← StoragePort, VideoProcessingPort
```
...
1. Pick one vertical slice (single endpoint/job).
2. Extract a use-case boundary with explicit input/output types.
3. Introduce outbound ports around infrastructure calls.
4. Move orchestration logic from controllers into the use case.
5. Add tests around the new boundary (unit + adapter integration).
6. Wire into `src/bootstrap/container.ts`.
...
- **Inbound adapter tests**: verify protocol mapping (HTTP payload to use-case input and output/error mapping back to protocol).
